document.addEventListener("DOMContentLoaded", function () {
    setTimeout(() => {
        addMessage("Bem-vindo ao ChatSAC, como posso te ajudar?", "bot");
    }, 500);
});
...
async function generateBotResponse(input) {
    try {
        console.log("Enviando JSON para a API:", { prompt: input }); // Loga o JSON
        const response = await fetch(`${BASE_URL}/chatSAC`, {
            method: 'POST',
            headers: {
        ...
        return data.answer; // Retorna a resposta da API
    } catch (error) {
        console.error("Erro no fetch:", error); // Loga o erro
